...
import { getMyParticipatedPublicPolls } from '@/lib/api/poll';
import type { MyPollResponse } from '@/lib/types/poll';

const PAGE_SIZE = 10;

export default function JoinedPublicPollsPage() {
	...
	const { data, isLoading, isError, isFetching } = useQuery<MyPollResponse>({
		queryKey: ['myParticipatedPublicPolls', page, PAGE_SIZE],
		queryFn: () =>
	...
	const handlePageChange = (nextPage: number) => {
		router.replace(
			{
				pathname: '/mypage/joined-public-polls',
				query: { page: nextPage },
			},
			undefined,
			{ shallow: true }
	...
	return (
		<>
			<Head>
				<title>참여한 모두의 투표 | KEP</title>
			</Head>
			<PaginatedPollListView
				title="참여한 모두의 투표"
				description="내가 참여한 모두의 투표 전체 목록을 볼 수 있어요."
				items={list}
				page={page}
